...
class BinGenTest{
  public:
    BinGenTest(std::ofstream ofs, std::ofstream coefs)
        : bingen_(BinGen(std::move(ofs), std::move(coefs), false /* is_verbose */, false /* is_debug */, false /* is_asci */)) {}
    ~BinGenTest() = default;
...
int main (void) {
    ...
    std::ofstream ofs, coefs;
    BinGenTest test(std::move(ofs), std::move(coefs));
    test.TestParse();
    test.TestConvert();
