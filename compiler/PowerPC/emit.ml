open Asm

external gethi : float -> int32 = "gethi"
external getlo : float -> int32 = "getlo"

let stackset = ref S.empty (* Set of variables already 'Saved' (caml2html: emit_stackset) *)
let stackmap = ref [] (* Positions of variables in the stack which are already 'Saved' (caml2html: emit_stackmap) *)
let save x =
  stackset := S.add x !stackset;
  if not (List.mem x !stackmap) then
    stackmap := !stackmap @ [x]
let savef x =
  (* TODO: change double-word floating point to single-word floating point *)
  stackset := S.add x !stackset;
  if not (List.mem x !stackmap) then
    (let pad =
       if List.length !stackmap mod 2 = 0 then [] else [Id.gentmp Type.Int] in
     stackmap := !stackmap @ pad @ [x; x])
let locate x =
  let rec loc = function
    | [] -> []
    | y :: zs when x = y -> 0 :: List.map succ (loc zs)
    | y :: zs -> List.map succ (loc zs) in
  loc !stackmap
let offset x = 4 * List.hd (locate x)
let stacksize () = align ((List.length !stackmap + 1) * 4)

let reg r =
  if is_reg r
  then String.sub r 1 (String.length r - 1)
  else r

let load_label r label =
  let r' = reg r in
  Printf.sprintf
    "\tlis\t%s, ha16(%s)\n\taddi\t%s, %s, lo16(%s)\n"
    r' label r' r' label

(* Shuffle registers so that the old content will not be lost *)
let rec shuffle sw xys =
  (* remove identical moves *)
  let xys = List.filter (fun (x, y) -> x <> y) xys in
  (* find acyclic moves *)
  match List.partition (fun (_, y) -> List.mem_assoc y xys) xys with
  | [], [] -> []
  | (x, y) :: xys, [] -> (* no acyclic moves; resolve a cyclic move *)
    (* there are a tuple (y, b) in old xys *)
    (y, sw) :: (x, y) :: shuffle sw (List.map
                                       (function
                                         | (y', z) when y = y' -> (sw, z)
                                         | yz -> yz)
                                       xys)
  | xys, acyc -> acyc @ shuffle sw xys

type dest = Tail | NonTail of Id.t (* 末尾かどうかを表すデータ型 (caml2html: emit_dest) *)
let rec g oc = function (* 命令列のアセンブリ生成 (caml2html: emit_g) *)
  | dest, Ans(exp) -> g' oc (dest, exp)
  | dest, Let((x, t), exp, e) ->
    g' oc (NonTail(x), exp);
    g oc (dest, e)
and g' oc = function (* 各命令のアセンブリ生成 (caml2html: emit_gprime) *)
  (* 末尾でなかったら計算結果をdestにセット (caml2html: emit_nontail) *)
  | NonTail(_), Nop -> ()
  | NonTail(x), Li(i) when -32768 <= i && i < 32768 -> Printf.fprintf oc "\tli\t%s, %d\n" (reg x) i
  | NonTail(x), Li(i) ->
    let n = i lsr 12 in
    let m = i lxor (n lsl 12) in
    let r = reg x in
    Printf.fprintf oc "\tlui\t%s, %d\n" r n;      (* ok *)
    Printf.fprintf oc "\tori\t%s, %s, %d\n" r r m (* ok *)
  | NonTail(x), FLi(Id.L(l)) ->
    let s = load_label (reg reg_tmp) l in
    Printf.fprintf oc "%s\tflw\t%s, 0(%s)\n" s (reg x) (reg reg_tmp) (* ok *)
  | NonTail(x), SetL(Id.L(y)) ->
    let s = load_label x y in
    Printf.fprintf oc "%s" s
  | NonTail(x), Mv(y) when x = y -> ()
  | NonTail(x), Mv(y) -> Printf.fprintf oc "\tmv\t%s, %s\n" (reg x) (reg y)                         (* ok *)
  | NonTail(x), Neg(y) -> Printf.fprintf oc "\tneg\t%s, %s\n" (reg x) (reg y)                       (* ok *)
  | NonTail(x), Add(y, V(z)) -> Printf.fprintf oc "\tadd\t%s, %s, %s\n" (reg x) (reg y) (reg z)     (* ok *)
  | NonTail(x), Add(y, C(z)) -> Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg x) (reg y) z          (* ok *)
  | NonTail(x), Sub(y, V(z)) -> Printf.fprintf oc "\tsub\t%s, %s, %s\n" (reg x) (reg y) (reg z)     (* ok *)
  | NonTail(x), Sub(y, C(z)) -> Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg x) (reg y) (-1 * z)   (* ok *)
  | NonTail(x), Slw(y, V(z)) -> Printf.fprintf oc "\tsll\t%s, %s, %s\n" (reg x) (reg y) (reg z)     (* ok *)
  | NonTail(x), Slw(y, C(z)) -> Printf.fprintf oc "\tslli\t%s, %s, %d\n" (reg x) (reg y) z          (* ok *)
  | NonTail(x), Lwz(y, V(z)) ->
    (* TODO: asml.ml, virtual.mlを変更してここが1命令になるようにする *)
    Printf.fprintf oc "\tadd\t%s, %s, %s\n" (reg reg_tmp) (reg y) (reg z);
    Printf.fprintf oc "\tlw\t%s, 0(%s)\n" (reg x) (reg reg_tmp) (* ok *)
  | NonTail(x), Lwz(y, C(z)) -> Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg x) z (reg y)            (* ok *)
  | NonTail(_), Stw(x, y, V(z)) ->
    Printf.fprintf oc "\tadd\t%s, %s, %s\n"(reg reg_tmp) (reg y) (reg z);
    Printf.fprintf oc "\tsw\t%s, 0(%s)\n" (reg x) (reg reg_tmp) (* ok *)
  | NonTail(_), Stw(x, y, C(z)) -> Printf.fprintf oc "\tsw\t%s, %d(%s)\n" (reg x) z (reg y)         (* ok *)
  | NonTail(x), FMv(y) when x = y -> ()
  | NonTail(x), FMv(y) -> Printf.fprintf oc "\tfmv.s\t%s, %s\n" (reg x) (reg y)                       (* ok *)
  | NonTail(x), FNeg(y) -> Printf.fprintf oc "\tfneg.s\t%s, %s\n" (reg x) (reg y)                     (* ok *)
  | NonTail(x), FAdd(y, z) -> Printf.fprintf oc "\tfadd.s\t%s, %s, %s\n" (reg x) (reg y) (reg z)      (* ok *)
  | NonTail(x), FSub(y, z) -> Printf.fprintf oc "\tfsub.s\t%s, %s, %s\n" (reg x) (reg y) (reg z)      (* ok *)
  | NonTail(x), FMul(y, z) -> Printf.fprintf oc "\tfmul.s\t%s, %s, %s\n" (reg x) (reg y) (reg z)      (* ok *)
  | NonTail(x), FDiv(y, z) -> Printf.fprintf oc "\tfdiv.s\t%s, %s, %s\n" (reg x) (reg y) (reg z)      (* ok *)
  | NonTail(x), Lfd(y, V(z)) ->
    Printf.fprintf oc "\tadd\t%s, %s, %s\n" (reg reg_tmp) (reg y) (reg z);
    Printf.fprintf oc "\tflw\t%s, 0(%s)\n" (reg x) (reg reg_tmp) (* ok *)
  | NonTail(x), Lfd(y, C(z)) -> Printf.fprintf oc "\tflw\t%s, %d(%s)\n" (reg x) z (reg y)             (* ok *)
  | NonTail(_), Stfd(x, y, V(z)) ->
    Printf.fprintf oc "\tadd\t%s, %s, %s\n" (reg reg_tmp) (reg y) (reg z);
    Printf.fprintf oc "\tfsw\t%s, 0(%s)\n" (reg x) (reg reg_tmp) (* ok *)
  | NonTail(_), Stfd(x, y, C(z)) -> Printf.fprintf oc "\tfsw\t%s, %d(%s)\n" (reg x) z (reg y)         (* ok *)
  | NonTail(_), Comment(s) -> Printf.fprintf oc "#\t%s\n" s
  (* 退避の仮想命令の実装 (caml2html: emit_save) *)
  | NonTail(_), Save(x, y) when List.mem x allregs && not (S.mem y !stackset) ->
    save y;
    Printf.fprintf oc "\tsw\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) (* ok *)
  | NonTail(_), Save(x, y) when List.mem x allfregs && not (S.mem y !stackset) ->
    savef y;
    Printf.fprintf oc "\tfsw\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) (* ok *)
  | NonTail(_), Save(x, y) -> assert (S.mem y !stackset); ()
  (* 復帰の仮想命令の実装 (caml2html: emit_restore) *)
  | NonTail(x), Restore(y) when List.mem x allregs ->
    Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) (* ok *)
  | NonTail(x), Restore(y) ->
    assert (List.mem x allfregs);
    Printf.fprintf oc "\tflw\t%s, %d(%s)\n" (reg x) (offset y) (reg reg_sp) (* ok *)
  (* 末尾だったら計算結果を%a0か%fa0にセットしてリターン (caml2html: emit_tailret) *)
  | Tail, (Nop | Stw _ | Stfd _ | Comment _ | Save _ as exp) ->
    g' oc (NonTail(Id.gentmp Type.Unit), exp);
    Printf.fprintf oc "\tjr\tra\n";
  | Tail, (Li _ | SetL _ | Mv _ | Neg _ | Add _ | Sub _ | Slw _ | Lwz _ as exp) ->
    g' oc (NonTail(regs.(0)), exp);
    Printf.fprintf oc "\tjr\tra\n";
  | Tail, (FLi _ | FMv _ | FNeg _ | FAdd _ | FSub _ | FMul _ | FDiv _ | Lfd _ as exp) ->
    g' oc (NonTail(fregs.(0)), exp);
    Printf.fprintf oc "\tjr\tra\n";
  | Tail, (Restore(x) as exp) ->
    (match locate x with
     | [i] -> g' oc (NonTail(regs.(0)), exp)
     | [i; j] when i + 1 = j -> g' oc (NonTail(fregs.(0)), exp)
     | _ -> assert false);
    Printf.fprintf oc "\tjr\tra\n";

    (* IF内がfalseの場合にjump *)
    (* TODO: 比較するものの一方が0のときは一命令減らせる *)
  | Tail, IfEq(x, V(y), e1, e2) ->
    g'_tail_if oc x y e1 e2 "beq" "bne"
  | Tail, IfEq(x, C(y), e1, e2) ->
    Printf.fprintf oc "\tli\t%s, %d\n" (reg reg_tmp) y;
    g'_tail_if oc x reg_tmp e1 e2 "beq" "bne"
  | Tail, IfLE(x, V(y), e1, e2) ->
    g'_tail_if oc x y e1 e2 "ble" "bgt"
  | Tail, IfLE(x, C(y), e1, e2) ->
    Printf.fprintf oc "\tli\t%s, %d\n" (reg reg_tmp) y;
    g'_tail_if oc x reg_tmp e1 e2 "ble" "bgt"
  | Tail, IfGE(x, V(y), e1, e2) ->
    g'_tail_if oc x y e1 e2 "bge" "blt"
  | Tail, IfGE(x, C(y), e1, e2) ->
    Printf.fprintf oc "\tli\t%s, %d\n" (reg reg_tmp) y;
    g'_tail_if oc x reg_tmp e1 e2 "bge" "blt"
  | Tail, IfFEq(x, y, e1, e2) ->
    (* Store the comparison result in reg_tmp (integer register!) *)
    Printf.fprintf oc "\tfeq.s\t%s, %s, %s\n" reg_tmp x y;
    (* x = y -> reg_tmpが1 -> beq reg_tmp zero で分岐しない *)
    g'_tail_if oc reg_tmp "zero" e1 e2 "bne" "beq"
  | Tail, IfFLE(x, y, e1, e2) ->
    Printf.fprintf oc "\tfle.s\t%s, %s, %s\n" reg_tmp x y;
    (* x <= y -> reg_tmpが1 -> beq reg_tmp zero で分岐しない *)
    g'_tail_if oc reg_tmp "zero" e1 e2 "bne" "beq"
  | NonTail(z), IfEq(x, V(y), e1, e2) ->
    g'_non_tail_if oc (NonTail(z)) x y e1 e2 "beq" "bne"
  | NonTail(z), IfEq(x, C(y), e1, e2) ->
    Printf.fprintf oc "\tli\t%s, %d\n" (reg reg_tmp) y;
    g'_non_tail_if oc (NonTail(z)) x reg_tmp e1 e2 "beq" "bne"
  | NonTail(z), IfLE(x, V(y), e1, e2) ->
    g'_non_tail_if oc (NonTail(z)) x y e1 e2 "ble" "bgt"
  | NonTail(z), IfLE(x, C(y), e1, e2) ->
    Printf.fprintf oc "\tli\t%s, %d\n" (reg reg_tmp) y;
    g'_non_tail_if oc (NonTail(z)) x reg_tmp e1 e2 "ble" "bgt"
  | NonTail(z), IfGE(x, V(y), e1, e2) ->
    g'_non_tail_if oc (NonTail(z)) x y e1 e2 "bge" "blt"
  | NonTail(z), IfGE(x, C(y), e1, e2) ->
    Printf.fprintf oc "\tli\t%s, %d\n" (reg reg_tmp) y;
    g'_non_tail_if oc (NonTail(z)) x reg_tmp e1 e2 "bge" "blt"
  | NonTail(z), IfFEq(x, y, e1, e2) ->
    Printf.fprintf oc "\tfeq.s\t%s, %s, %s\n" reg_tmp x y;
    g'_non_tail_if oc (NonTail(z)) reg_tmp "zero" e1 e2 "bne" "beq"
  | NonTail(z), IfFLE(x, y, e1, e2) ->
    Printf.fprintf oc "\tfle.s\t%s, %s, %s\n" reg_tmp x y;
    g'_non_tail_if oc (NonTail(z)) reg_tmp "zero" e1 e2 "bne" "beq"

  (* 関数呼び出しの仮想命令の実装 (caml2html: emit_call) *)
  | Tail, CallCls(f, iargs, fargs) -> (* 末尾呼び出し (caml2html: emit_tailcall) *)
    (* TODO: tレジスタから使うようにする(?) *)
    g'_args oc [(f, reg_cl)] iargs fargs;
    Printf.fprintf oc "\tlw\t%s, 0(%s)\n" (reg reg_sw) (reg reg_cl);
    Printf.fprintf oc "\tmv\tra, %s\n" (reg reg_sw);
    Printf.fprintf oc "\tjr\tra\n"
  | Tail, CallDir(Id.L(f), iargs, fargs) -> (* 末尾呼び出し *)
    g'_args oc [] iargs fargs;
    Printf.fprintf oc "\tj\t%s\n" f
  | NonTail(a), CallCls(f, iargs, fargs) ->
    Printf.fprintf oc "\tmv\t%s, %s\n" (reg reg_tmp) (reg reg_link);
    g'_args oc [(f, reg_cl)] iargs fargs;
    let ss = stacksize () in
    Printf.fprintf oc "\tsw\t%s, %d(%s)\n" (reg reg_tmp) (ss - 4) (reg reg_sp); (* evacuate link register *)
    Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg reg_fp) (reg reg_sp) ss;
    Printf.fprintf oc "\tlw\t%s, 0(%s)\n" (reg reg_tmp) (reg reg_cl);
    Printf.fprintf oc "\tmv\t%s, %s\n" (reg reg_link) (reg reg_tmp); (* set closure address to %ra *)
    Printf.fprintf oc "\tjr\tra\n";
    Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg reg_fp) (reg reg_sp) (-1 * ss);
    Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg reg_tmp) (ss - 4) (reg reg_sp);
    if List.mem a allregs && a <> regs.(0) then
      Printf.fprintf oc "\tmv\t%s, %s\n" (reg a) (reg regs.(0))
    else if List.mem a allfregs && a <> fregs.(0) then
      Printf.fprintf oc "\tfmv.s\t%s, %s\n" (reg a) (reg fregs.(0));
    Printf.fprintf oc "\tmv\t%s, %s\n" (reg reg_link) (reg reg_tmp)
  | (NonTail(a), CallDir(Id.L(f), iargs, fargs)) ->
    Printf.fprintf oc "\tmv\t%s, %s\n" (reg reg_tmp) (reg reg_link);
    g'_args oc [] iargs fargs;
    let ss = stacksize () in
    Printf.fprintf oc "\tsw\t%s, %d(%s)\n" (reg reg_tmp) (ss - 4) (reg reg_sp);
    Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg reg_fp) (reg reg_sp) ss;
    Printf.fprintf oc "\tcall\t%s\n" f;
    Printf.fprintf oc "\taddi\t%s, %s, %d\n" (reg reg_fp) (reg reg_sp) (-1 * ss);
    Printf.fprintf oc "\tlw\t%s, %d(%s)\n" (reg reg_tmp) (ss - 4) (reg reg_sp);
    if List.mem a allregs && a <> regs.(0) then
      Printf.fprintf oc "\tmv\t%s, %s\n" (reg a) (reg regs.(0))
    else if List.mem a allfregs && a <> fregs.(0) then
      Printf.fprintf oc "\tfmv.s\t%s, %s\n" (reg a) (reg fregs.(0));
    Printf.fprintf oc "\tmv\t%s, %s\n" (reg reg_link) (reg reg_tmp)
and g'_tail_if oc rs1 rs2 e1 e2 b bn =
  let b_else = Id.genid (b ^ "_else") in
  Printf.fprintf oc "\t%s\t%s, %s, %s\n" bn (reg rs1) (reg rs2) b_else;
  let stackset_back = !stackset in
  g oc (Tail, e1); (* if内がtrueの場合 = jumpしなかった場合 *)
  Printf.fprintf oc "%s:\n" b_else;
  stackset := stackset_back;
  g oc (Tail, e2)
and g'_non_tail_if oc dest rs1 rs2 e1 e2 b bn =
  let b_else = Id.genid (b ^ "_else") in
  let b_cont = Id.genid (b ^ "_cont") in
  Printf.fprintf oc "\t%s\t%s, %s, %s\n" bn rs1 rs2 b_else;
  let stackset_back = !stackset in
  g oc (dest, e1);
  let stackset1 = !stackset in
  Printf.fprintf oc "\tb\t%s\n" b_cont;
  Printf.fprintf oc "%s:\n" b_else;
  stackset := stackset_back;
  g oc (dest, e2);
  Printf.fprintf oc "%s:\n" b_cont;
  let stackset2 = !stackset in
  stackset := S.inter stackset1 stackset2
and g'_args oc x_reg_cl int_args float_args =
  let (_, yrs) =
    List.fold_left
      (fun (i, yrs) y -> (i + 1, (y, regs.(i)) :: yrs))
      (0, x_reg_cl) (* <- initial value *)
      int_args in
  List.iter
    (fun (y, r) -> Printf.fprintf oc "\tmv\t%s, %s\n" (reg r) (reg y))
    (shuffle reg_sw yrs);
  let (_, zfrs) =
    List.fold_left
      (fun (d, zfrs) z -> (d + 1, (z, fregs.(d)) :: zfrs))
      (0, [])
      float_args in
  List.iter
    (fun (z, fr) -> Printf.fprintf oc "\tfmv.s\t%s, %s\n" (reg fr) (reg z))
    (shuffle reg_fsw zfrs)

let h oc { name = Id.L(x); args = _; fargs = _; body = e; ret = _ } =
  Printf.fprintf oc "%s:\n" x;
  stackset := S.empty;
  stackmap := [];
  g oc (Tail, e)

let f oc (Prog(data, fundefs, e)) =
  Format.eprintf "generating assembly...@.";
  if data <> [] then
    (* TODO: ここらへんのアセンブラ対応 *)
    (Printf.fprintf oc "\t.data\n\t.literal8\n";
     List.iter
       (fun (Id.L(x), d) ->
          Printf.fprintf oc "\t.align 3\n";
          Printf.fprintf oc "%s:\t # %f\n" x d;
          Printf.fprintf oc "\t.long\t%ld\n" (gethi d);
          Printf.fprintf oc "\t.long\t%ld\n" (getlo d))
       data);
  Printf.fprintf oc "\t.text\n";
  Printf.fprintf oc "\t.globl _min_caml_start\n";
  Printf.fprintf oc "\t.align 2\n";
  List.iter (fun fundef -> h oc fundef) fundefs;
  Printf.fprintf oc "_min_caml_start: # main entry point\n";
  Printf.fprintf oc "\taddi\tsp, sp, -16\n";
  Printf.fprintf oc "\tsw\tra, 8(sp)\n";
  Printf.fprintf oc "\tsw\tfp, 0(sp)\n";
  Printf.fprintf oc "\taddi\tfp, sp, 16\n";
  Printf.fprintf oc "#\tmain program starts\n";
  stackset := S.empty;
  stackmap := [];
  g oc (NonTail("_R_0"), e);
  Printf.fprintf oc "#\tmain program ends\n";
  Printf.fprintf oc "\tli\ta5, 0\n";
  Printf.fprintf oc "\tmv\ta0, a5\n";
  Printf.fprintf oc "\tlw\tra, 8(sp)\n";
  Printf.fprintf oc "\tlw\tfp, 0(sp)\n";
  Printf.fprintf oc "\taddi\tsp, sp, 16\n";
  Printf.fprintf oc "\tjr\tra\n";
