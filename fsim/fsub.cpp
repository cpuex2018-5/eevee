...
float fsub(unsigned char *ovf,float x1_f,float x2_f){
  ...
  unsigned int te = get_binary((0b100000000+e1a)+(0b100000000+(~e2a)),0,9); //9bitなので計算した後切り出す

  unsigned int te2 = get_binary((te+0b0000000001),0,9); //9bitなので計算した後切り出す
  unsigned int tde = (get_binary(te,8,9)==1) ? get_binary(te2,0,8) : ~get_binary(te,0,8); //9bit

  unsigned int de = (tde>31) ? 0b11111 : get_binary(tde,0,5); //5bit

  unsigned int sel = (de==0) ? ((m1a > m2a)? 0 : 1) : ~get_binary(te,8,9); //1bit
